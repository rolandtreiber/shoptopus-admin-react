...
export const DiscountRuleMenu = (props) => {
  ...
  const {fetchDiscountRule, updateDiscountRule, deleteDiscountRule} = useContext(APIContext)
  ...
  const doDelete = useCallback( async (id) => {
    try {
      return await deleteDiscountRule(id);
    } catch (err) {
      console.error(err);
    }
  }, [])
  ...
  const handleDelete = useCallback(async () => {
    ...
    setDescription('You are about to delete a discount rule.')
    showGenericDialog(true)
  }, [id])

  const doStatusChange = async (id, status) => {
    try {
      return await updateDiscountRule(id, {
        enabled: status
      });
    } catch (err) {
      console.error(err);
  ...
  const handleStatusChange = (currentStatus) => {
    ...
    setCallback({method: call})
    setTitle('Are you sure?')
    setDescription('You are about to '+text+' a discount rule.')
    showGenericDialog(true)
  };

  const getDiscountRule = useCallback(async () => {
    if (id) {
      try {
        const result = await fetchDiscountRule(id)
